...
def run_i456(dict_cfg: dict, hydra_run_dir) -> None:
    ...
    if 'i6' in jobs_to_run:
        ...
        evaluate.evaluate()
        logging.info(f'i5_evaluate_annotations took {(time.time() - t0) / 3600:.2f} h')
    ...
    if 'i5' in jobs_to_run:
        ...
        logging.info(f'i6_extract_segmentation took {(time.time() - t0) / 3600:.2f} h')
...
def main(cfg: DictConfig) -> None:
    ...
    logging.info(f'Finished all tasks ({jobs_to_run}) in {hours:.2f} h')
